...
_ : compile-eval "0 + 0" ≡ inj₁ (Nat , lam (lam (iteNat q (suco q) (q [ p ]))) $      zeroo $      zeroo , λ γ* → 0)
_ = refl
_ : compile-eval "0 + 1" ≡ inj₁ (Nat , lam (lam (iteNat q (suco q) (q [ p ]))) $      zeroo $ suco zeroo , λ γ* → 1)
_ = refl
_ : compile-eval "1 + 0" ≡ inj₁ (Nat , lam (lam (iteNat q (suco q) (q [ p ]))) $ suco zeroo $      zeroo , λ γ* → 1)
_ = refl
_ : compile-eval "2 + 3" ≡ inj₁ (Nat , lam (lam (iteNat q (suco q) (q [ p ]))) $ suco (suco zeroo) $ suco (suco (suco zeroo))
                                     , λ γ* → 5)
_ = refl

-- associativity of _+_ gives us chains, it associates to the left by default:

_ : compile-eval "0 + 1 + 2" ≡ inj₁ (Nat , lam (lam (iteNat q (suco q) (q [ p ]))) $ (lam (lam (iteNat q (suco q) (q [ p ])))
                                           $ zeroo $ suco zeroo) $ suco (suco zeroo)
                                         , (λ γ* → 3))
_ = refl

-- but we can, of course, parenthesize differently:

_ : compile-eval "0 + (1 + 2)" ≡ inj₁ (Nat , lam (lam (iteNat q (suco q) (q [ p ]))) $ zeroo
                                             $ (lam (lam (iteNat q (suco q) (q [ p ]))) $ suco zeroo $ suco (suco zeroo))
                                           , (λ γ* → 3))
_ = refl

-- combined usage of the two destructors

_ : compile-eval "isZero 0 + 0" ≡ inj₁ (Bool , iteNat true false (lam (lam (iteNat q (suco q) (q [ p ]))) $ zeroo $ zeroo)
                                             , (λ γ* → tt))
_ = refl
_ : compile-eval "isZero 1 + 0" ≡ inj₁ (Bool , iteNat true false (lam (lam (iteNat q (suco q) (q [ p ]))) $ suco zeroo $ zeroo)
                                             , λ γ* → ff)
_ = refl
...
_ : compile-eval triple ≡ inj₁ (Nat ⇒ Nat , lam (lam (lam (iteNat q (suco q) (q [ p ])))
                                          ...
                                          , λ γ* x → iteℕ x (λ z → suc z) (iteℕ x (λ z → suc z) x))
_ = refl

-- we can implement _+_ with a function using the iterator of ℕ
_ : compile-eval plus ≡ inj₁ (Nat ⇒ Nat ⇒ Nat
               , lam (lam (iteNat (q [ p ]) (lam (lam (lam (iteNat q (suco q) (q [ p ]))) $ q $ suco zeroo) [ p ] $ q) q))
               , λ γ* x y → iteℕ x (λ z → iteℕ 1 (λ w → suc w) z) y)
_ = refl

-- our language does not have built-in multiplication, but we can implement one:
_ : compile-eval multiply ≡ inj₁ (Nat ⇒ Nat ⇒ Nat , lam (lam (iteNat zeroo (lam (lam (lam (iteNat q (suco q) (q [ p ])))
                                                    $ q $ q [ p ] [ p ]) [ p ] $ q) q))
